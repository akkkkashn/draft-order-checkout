...
// Lock CORS to your storefront origins
...
function parsePrice(value) {
  ...
  return num.toFixed(2); // major units as string
}
...
// Normalize incoming properties (object or array) into Shopify Admin API format: [{ name, value }]
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Expect payload from theme:
    // { productTitle, properties, imageUrl?, quantity, customPrice, customerEmail?, note? }
    const {
      productTitle,
      properties,      // user-selected configuration only (theme already filtered)
      imageUrl,        // optional URL for the configured image (will be added as a property)
      quantity = 1,
      customPrice,
      customerEmail,
      note
    ...
    // Normalize properties and add imageUrl as a property (optional)
    const propsArray = normalizeProperties(properties);
    if (imageUrl && !propsArray.some(p => p.name.toLowerCase() === 'image url')) {
      propsArray.push({ name: 'Image URL', value: String(imageUrl) });
    }

    // Build a true custom line item (no variant_id) so Shopify uses your price
    const draftOrderData = {
      draft_order: {
        line_items: [
          {
            title: productTitle || 'Custom Item',
            price: priceStr,
            quantity: qty,
